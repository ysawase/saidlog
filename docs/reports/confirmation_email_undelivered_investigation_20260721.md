...
保留4項目1位「確認メール未達」は解決した。

- Resend送信ドメイン mail.solidaily.com を認証し、Supabase
  Authentication → Emails → SMTP Settings のCustom SMTPへ接続した
- Sender: SaidLog <no-reply@mail.solidaily.com>
- Supabase Site URL が localhost:3000 のままになっていた問題も
  合わせて発見・修正した（本番URLへ変更）
- E2E確認：新規登録 → Supabase Auth → Resend SMTP → 確認メール到着
  → メール認証 → 本番環境へログイン、の一連が成功
- テストユーザーはログアウト後にSupabaseから削除、残存ゼロを確認済み

上記「推定原因」「次のアクション」で未実測としていた項目は、この対応
により解消されたものとして扱う。
