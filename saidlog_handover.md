...
### STTエンジン方針（2026-06-22 更新）
- 標準STT：Groq（Whisper v3 Turbo）に切り替え予定
- AmiVoiceは将来の松プラン・追加課金候補として維持
- AssemblyAIは現時点では保留（無料プランはモデル学習オプトアウト不可・有料移行で解決）
- STT抽象化層は引き続き維持する
- Groqは話者分離なし。話者名の自動断定は切り捨てる方針
- 話者ラベルはA/B/C表示・ユーザー編集可能の設計を維持
- AssemblyAIの7月値上げ（10%）は考慮済み
