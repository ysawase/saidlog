...
### 短い説明文
録音するだけ。AIが会議メモに整理します。

### 長い説明文
会議の録音を、AIが要点・決定事項・TODOに整理します。
聞き返さなくても、あとで確認できます。

■ こんなときに
✓ 録音したけど聞き返す時間がない
✓ 会議後の決定事項・TODO、あとで見返したい

■ できること
・スマホで録音、または音声ファイルをアップロード
・AIが要点・決定事項・TODOを整理
・文字起こし全文を確認
・TXT・DOCX・PDFで保存（竹プラン）
・会議履歴を保存・管理

■ 対面会議・打ち合わせにも
オンライン会議ツールを使わない打ち合わせにも使えます。
会議室・カフェ・訪問先でスマホを置いて録音するだけ。

■ プラン
無料：月60分まで試せます
・要点を一部確認／履歴3件／エクスポート不可

竹プラン：月680円
・月10時間／詳細AI要約／PDF・DOCX・TXT保存／履歴30件

■ ご注意
・AIの整理結果は内容をご確認のうえご利用ください
・録音環境により精度が変わる場合があります
・話者ごとの発言整理には対応していません
・オンライン会議ツールとの連携機能はありません

### スクリーンショットキャプション（5枚）
1枚目：録音した会議、聞き返さなくていい。
2枚目：AIが会議メモに整理します。
3枚目：決定事項とTODOが残ります。
4枚目：PDF・DOCX・TXTで保存できます。
5枚目：月680円で、会議メモを残して出せる。

### 期待値ズレ対策方針
「AI議事録」タイトルによる期待値ズレは短い説明文・長い説明文・スクショで補正する。
公開後に「話者識別がない」「正式議事録ではない」というレビューが多ければタイトル変更を検討する。

## マーケティング・訴求決定事項（2026-06-29確定）

### ブランドポジション
AI会議メモ（「AI議事録」は検索補助語として使うがブランドの中心には置かない）

### キャッチコピー
「会議の録音をAIで整理。あとで使える会議メモに。」

### ターゲット優先順位
1. 個人事業主・フリーランス（自腹課金の決断が早い・680円との相性がよい）
2. クライアント打ち合わせ・商談が多い会社員
3. 会議後のメモ整理が面倒な会社員

### 訴求で使う言葉
主軸：AI会議メモ・会議録音・AI要約・要点・決定事項・TODO・あとで使える・録音をAIで整理
シーン：対面会議・リアル会議・対面打ち合わせ・オンライン会議ツールを使わない打ち合わせにも
検索補助：AI議事録・議事録アプリ・文字起こし

### 使わない言葉
話者識別・話者分離・完全な議事録・高精度・チーム共有・Zoom連携・Google Meet連携・リアルタイム・一気通貫

### 差別化軸
「リアル会議・対面打ち合わせ」はスクショ・長説明文でシチュエーションとして表現。
「Zoom不要」は直接言わず「オンライン会議ツールを使わない打ち合わせにも」と表現。

### 無料代替への対抗軸
「手間の削減」ではなく「会議記録が毎回同じ型で残り、後から使える」

### プラン別訴求
- 梅訴求：録音から要点・決定事項・TODOを確認（無料：月60分まで試せる）
- 竹訴求：月680円で、会議メモを残して出せる

### 課金壁の原則
ユーザーはプラン表を読まない。自分がやりたい操作を止められた瞬間に課金理由を理解する。
- 悪い：「竹プランで使えます」
- 良い：「PDFで保存するには竹プランが必要です」→ボタン「竹プランで保存する」

### 話者識別の方針（2026-06-29確定）
- 初期リリース：Groq継続・話者識別なし
- 理由：担当者断定は話者識別では解決しない／AssemblyAI日本語品質リスク／AmiVoiceは竹680円に収まらない
- 話者識別の価値：担当者自動断定ではなく「要約後に原文確認する作業を助けること」
- 将来：松プラン候補（2,980円/月・5時間・AmiVoice・話者識別あり）
- 追加判断条件：要約後に文字起こし確認へ戻るユーザーが多い／「誰が話したか分からない」不満が竹ユーザーに多い

## 効果検証設計（2026-06-29確定）

### 基本思想
管理画面を見に行かず、判断材料が届く仕組み。数字ではなく「判断候補」を届ける。

### Phase1：知人10人程度
自動化なし。手動メモ+既存Supabaseログ。
確認項目：
- 要約まで到達したか
- 処理失敗がないか
- 原価が想定内か
...
2. summary_completed
3. summary_viewed
4. transcript_viewed_after_summary
5. export_clicked_free
6. copy_clicked_free
7. history_limit_hit
8. paywall_viewed
9. purchase_clicked
...
処理失敗管理（イベントではなくjob側で管理）：
- status: submitted / processing / completed / failed
- error_stage / error_code
- created_at / completed_at / failed_at

イベント共通属性：
user_id / meeting_id / user_plan / input_type / created_at

週次でSaidLog Weekly Decision ReportをResend（無料）でメール送信。

レポート構成：
① 今週の判定候補（GO / HOLD / FIX / 母数不足）
② 判定理由（3行以内）
③ 今週やること1つ
④ 今週判断しないこと（原則）
⑤ 今週の変化点（異常値のみ）
⑥ 主要数字
⑦ Decision Log（重要判断週はObsidianにもコピー）
⑧ AI分析：↓をコピーしてClaudeへ（テンプレートは別ファイル管理）

GO/HOLD/FIX定義：
- GO：次の母数を取りに行ってよい。やること：知人を増やす・告知準備。やらないこと：価格・機能・訴求・ストアの変更
- HOLD：観察継続。やること：今週やること1つだけ実行。やらないこと：複数変更を同時にしない
- FIX：詰まりがある。やること：原因を1つ特定して直す。やらないこと：集客・新機能追加
- 母数不足（10件未満）：判定候補を出さない。やること：知人への声かけを1件増やす

### Phase2b：自動AI分析追加
移行条件（運用反復性ベース）：
1. Weekly Decision Reportが4週連続で届いている
2. AI分析テンプレートを3回以上実際に使っている
3. Decision Logが2〜3回残っている
4. AI分析結果が毎回似た観点に収束している
5. 手動貼り付けが面倒だと感じ始めている
...
### コスト
月4〜12円（Phase2b含む）。メール送信：Resend無料枠（月3,000通）
